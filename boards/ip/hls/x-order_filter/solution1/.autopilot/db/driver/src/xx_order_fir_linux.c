...
typedef struct {
    ...
    XX_order_fir_uio_map maps[ MAX_UIO_MAPS ];
} XX_order_fir_uio_info;

/***************** Variable Definitions **************************************/
static XX_order_fir_uio_info uio_info;
...
static int uio_info_read_name(XX_order_fir_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XX_order_fir_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XX_order_fir_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XX_order_fir_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XX_order_fir_Initialize(XX_order_fir *InstancePtr, const char* InstanceName) {
	XX_order_fir_uio_info *InfoPtr = &uio_info;
